...
namespace JackboxGPT3.Games.Common
{
  public abstract class BaseJackboxClient<TRoom, TPlayer> : IJackboxClient
  {
    ...
    public void Connect(string player_name, string room_code)
    {
      ...
      var url = new Uri($"wss://{_configuration.EcastHost}/api/v2/rooms/{room_code}/play?{bootstrap.AsQueryString()}");
    ...
    protected void WsSend<T>(string opCode, T body)
    {
      ...
      if (_webSocket == null)
      {
        throw new NullReferenceException("Websocket is null");
      }
      _webSocket.Send(msg);
    }
    ...
    public GameStatus GetGameStatus()
    {
      if (_exitEvent == null)
      {
        return GameStatus.Disconnection;
      }

      return _exitEvent.WaitOne(0) ? GameStatus.Disconnection : GameStatus.Connected;
    }
  }
}
